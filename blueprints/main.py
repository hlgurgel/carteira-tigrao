...
def _get_summary(account, year, month=None):
    ...
    def q_sum(cat_id, y, m=None):
        ...
        if m:
            q = q.filter(extract("month", Transaction.date) == m)
        result = q.scalar()
        return Decimal(str(result)) if result else Decimal("0")

    total_income = q_sum(income_cat.id, year, month) if income_cat else Decimal("0")
    ...
    for cat in cats:
        if cat.category_type == "income":
            ...
            allocated = (total_income * Decimal(str(cat.percentage)) / 100).quantize(Decimal("0.01"))
            spent = q_sum(cat.id, year, month)
...
def dashboard():
    ...
    if view == "month":
        if month == 1:
            prev = {"year": year - 1, "month": 12, "view": "month"}
        else:
            prev = {"year": year, "month": month - 1, "view": "month"}
        if month == 12:
            nxt = {"year": year + 1, "month": 1, "view": "month"}
        else:
            nxt = {"year": year, "month": month + 1, "view": "month"}
    else:
    ...
    period_label = (
        f"{month_names[month - 1]} {year}" if view == "month" else str(year)
    )

    # Resumo anual por mês (para gráfico na visão anual)
    monthly_data = []
    if view == "year":
        for m in range(1, 13):
...
def add_transaction():
    ...
    errors = []
    if not amount_str:
        errors.append("Informe o valor.")
    if not tag_name:
        errors.append("Informe a tag.")
    if not cat_id:
